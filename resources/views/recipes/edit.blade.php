                    ...
                    <form action="{{ route('recipes.update', $recipe) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="ingredients-wrapper">
                                @foreach (is_array($recipe->ingredients) ? $recipe->ingredients : [] as $i => $ingredient)
                                    <div class="input-group mb-2 ingredient-item">
                                        <input type="text" name="ingredients[{{ $i }}][name]" class="form-control" placeholder="Nombre del ingrediente" value="{{ $ingredient['name'] }}" required>
                                        <input type="text" name="ingredients[{{ $i }}][quantity]" class="form-control" placeholder="Cantidad" value="{{ $ingredient['quantity'] }}" required>
                                        <input type="text" name="ingredients[{{ $i }}][unit]" class="form-control" placeholder="Unidad (ej. gr, ml)" value="{{ $ingredient['unit'] ?? '' }}">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="steps-wrapper">
                                @foreach (is_array($recipe->steps) ? $recipe->steps : [] as $i => $step)
                                    <div class="input-group mb-2 step-item">
                                        <span class="input-group-text">Paso {{ $i + 1 }}</span>
                                        <textarea name="steps[{{ $i }}][description]" class="form-control" rows="2" required>{{ $step['description'] }}</textarea>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control @error('prep_time') is-invalid @enderror" id="prep_time" name="prep_time" value="{{ old('prep_time', $recipe->prep_time) }}" required>
                                @error('prep_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control @error('cook_time') is-invalid @enderror" id="cook_time" name="cook_time" value="{{ old('cook_time', $recipe->cook_time) }}" required>
                                @error('cook_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control @error('servings') is-invalid @enderror" id="servings" name="servings" value="{{ old('servings', $recipe->servings) }}" required>
                                @error('servings')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="category" name="category" required>
                                    ...
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat }}" @if(old('category', $recipe->category) == $cat) selected @endif>{{ $cat }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            @if($recipe->image)
                                <div class="mb-2">
                                    <img src="{{ Storage::url($recipe->image) }}" alt="Imagen actual" class="img-fluid rounded" style="max-height: 150px;">
                                </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('group_id') is-invalid @enderror" id="group_id" name="group_id">
                                ...
                                @foreach($allGroups as $group)
                                    <option value="{{ $group->id }}" {{ old('group_id', $recipe->group_id) == $group->id ? 'selected' : '' }}>
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" value="1" id="is_private" name="is_private" {{ old('is_private', $recipe->is_private) ? 'checked' : '' }}>
                        ...
                        <a href="{{ route('recipes.show', $recipe) }}" class="btn btn-secondary">Cancelar</a>
                    </form>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let ingredientIndex = ingredientsWrapper.children.length;
    ...
    let stepIndex = stepsWrapper.children.length;
